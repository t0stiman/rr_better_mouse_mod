    ...
    switch (__instance.CurrentCharacterState)
    {
      ...
      case CharacterState.Ladder:
        ...
        if (__instance.IsMovingInto(__instance._ladder.CapsuleCollider, __instance._moveInputVector, true))
        {
          ...
          var vector3_2 = __instance._ladder.transform.up * (Time.fixedDeltaTime * (Mathf.Lerp(0.0f, __instance._inputRun ? __instance.ladderSpeedFast : __instance.ladderSpeedNormal, Mathf.InverseLerp(0.0f, 1f, __instance._ladderDuration)) * __instance._ladder.SpeedMultiplierForPosition(__instance._ladderLocalPosition)));
